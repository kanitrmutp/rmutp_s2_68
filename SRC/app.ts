...
app.post("/encode", async (c) => {
  const body = await c.req.json();
  const dataToEncode = body.data; // รับข้อมูลจาก field 'data' ใน JSON body
  const encodedData = encode(dataToEncode);
  return c.json({
...
app.post("/decode", async (c) => {
  const body = await c.req.json();
  const dataToDecode = body.data; // รับข้อมูลจาก field 'data' ใน JSON body
  const decodedData = decode(dataToDecode);
  return c.json({
